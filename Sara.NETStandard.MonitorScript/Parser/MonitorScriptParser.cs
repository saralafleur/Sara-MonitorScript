...
namespace Sara.NETStandard.MonitorScript.Parser
{
    public sealed partial class MonitorScriptParser
    {
        ...
        private int _index;
        private MonitorScriptParserOptions _options;
        ...
        public MonitorScriptParser()
            : this(new ErrorSink())
        {
        }

        public MonitorScriptParser(ErrorSink errorSink)
        {
        ...
        private SyntaxException UnexpectedToken(string expected)
        {
            ...
            if (expected[0] == '\'')
                message = $"Unexpected '{value}'.  Expected {expected}";
            else
                message = $"Unexpected '{value}'.  Expected '{expected}'";
